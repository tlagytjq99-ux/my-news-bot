...
def main():
    ...
    try:
        ...
        # 💡 전략 변경: h태그뿐만 아니라 모든 링크(a)와 텍스트 박스를 다 뒤집니다.
        pwc_count = 0
        potential_items = soup.find_all(['a', 'h3', 'h4', 'span'])
        
        for item in potential_items:
            text = item.get_text().strip()
            # 1. 제목이 20자 이상 150자 이하인 '문장' 형태만 필터링
            if 20 < len(text) < 150:
                # 2. 테크 관련 핵심 키워드 포함 여부 (PwC 핵심 키워드 위주)
                if any(kw in text.upper() for kw in ['AI', 'GEN', 'DIGITAL', 'INTELLIGENCE', 'TECH', 'CLOUD', 'DATA']):
                    
                    # 링크 찾기: 현재 요소가 a면 바로 쓰고, 아니면 부모나 자식에서 찾음
                    link = ""
                    if item.name == 'a': link = item.get('href', '')
                    else:
                        ...
                        if not any(d['원문'] == text for d in new_data):
                            try:
                                ...
                                new_data.append({
                                    ...
                                    "제목": title_ko, "원문": text, "링크": full_url, "수집일": collected_date
                                })
                                pwc_count += 1
